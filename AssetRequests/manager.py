...
class RequestManager(models.Manager):
    ...
    def all_pending_requests(self):
        ...
        return super().get_queryset().filter(mgr_status = 'pending').order_by('-requestdate')
    
    def all_cancelled_requests(self):
        return super().get_queryset().filter(mgr_status = 'cancelled').order_by('-requestdate')
    
    
    def all_rejected_requests(self):
        return super().get_queryset().filter(mgr_status = 'rejected').order_by('-requestdate')
    
    def all_approved_requests(self):
        
        '''get all approved requests'''
        return super().get_queryset().filter(mgr_status = 'confirmed').order_by('-requestdate')
    
    def cto_all_pending_requests(self):
        return super().get_queryset().filter(mgr_status='confirmed', cto_status = 'pending').order_by('-requestdate') #using the mgr_status for definition. Approved by Manager goes to CTO waiting list
